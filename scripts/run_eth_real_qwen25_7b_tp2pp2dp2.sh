...
if [[ "${HOSTNAME}" != "sd-1" ]]; then
    echo "[Error] Run this script from sd-1 so it can launch the dual-node job." >&2
    exit 1
fi

# Add conda env to PATH (both local and remote will use this)
...
export PYTHON_BIN="${PYTHON_BIN:-/home/user/miniconda3/envs/tp4bit/bin/python3}"
export LAUNCHER="${LAUNCHER:-deepspeed}"

export EXPERIMENT_NAME="${EXPERIMENT_NAME:-eth_real_qwen25_7b_tp2pp2dp2_baseline_formal20}"
...
export MASTER_PORT="${MASTER_PORT:-32031}"
export HOSTFILE="${HOSTFILE:-/tmp/hostfile_eth_real_qwen25_7b_tp2pp2dp2}"
...
# Conda environment for remote nodes (critical for sd-2)
export CONDA_ENV="${CONDA_ENV:-tp4bit}"

# Ethernet network settings (no IB)
...
# Use HF cache tokenizer (same as successful 1650 run)
