#!/bin/bash
# Qwen2.5-7B Single Node Training with STATIC 1597MHz
# 全程锁定 1597MHz（最高频率）
set -ex
...
CHECKPOINT_PATH=${BASE_PATH}/checkpoints/qwen7b_tp4_static_1597
...
# 训练开始前锁定所有 GPU 到 1597MHz
echo "Locking all GPUs to 1597MHz..."
python3 -c "
import sys
...
for i in range(16):
    ...
    pynvml.nvmlDeviceSetGpuLockedClocks(h, 1597, 1597)
    print(f'GPU {i} locked to 1597MHz')
pynvml.nvmlShutdown()
"
...
echo "Single node training (STATIC 1597MHz): TP=$TP, PP=$PP, DP=$((WORLD_SIZE / TP / PP))"
