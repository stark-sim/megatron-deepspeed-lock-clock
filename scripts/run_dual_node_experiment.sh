...
# 验证 TP * PP * DP = WORLD_SIZE
if (( TP * PP * DP != WORLD_SIZE )); then
    echo "错误: TP($TP) * PP($PP) * DP($DP) = $((TP*PP*DP)) != WORLD_SIZE($WORLD_SIZE)"
    exit 1
fi
...
echo "localhost slots=$GPUS_PER_NODE" > "$HOSTFILE"
echo "192.168.205.202 slots=$GPUS_PER_NODE" >> "$HOSTFILE"

# DeepSpeed 配置
DS_CONFIG="/tmp/ds_config_$$.json"
cat > "$DS_CONFIG" << EOF
{
  "train_batch_size": 16,
