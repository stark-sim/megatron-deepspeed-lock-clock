#!/usr/bin/env bash
# Convert LLaMA-7B HF weights to Megatron-DeepSpeed format on V100
# Usage: bash scripts/convert_llama7b_hf_to_megatron_v100.sh [TP] [PP] [GPU_START] [GPU_END]
#   TP: tensor parallel size (default: 2)
#   PP: pipeline parallel size (default: 2)
#   GPU_START/GPU_END: GPU range for conversion (default: 0 to TP*PP-1)
#
# Example:
#   bash scripts/convert_llama7b_hf_to_megatron_v100.sh 2 2    # TP2PP2 on GPU 0-3
#   bash scripts/convert_llama7b_hf_to_megatron_v100.sh 1 2    # TP1PP2 on GPU 0-1
#   bash scripts/convert_llama7b_hf_to_megatron_v100.sh 2 1    # TP2PP1 on GPU 0-1
#   bash scripts/convert_llama7b_hf_to_megatron_v100.sh 4 1    # TP4PP1 on GPU 0-3
#   bash scripts/convert_llama7b_hf_to_megatron_v100.sh 1 4    # TP1PP4 on GPU 0-3
...
HF_MODEL="${HF_MODEL:-/home/sd/models/llama-7b-hf}"
...
RUN_ID="llama7b_hf2megads_tp${TP}pp${PP}_$(date +%Y%m%d_%H%M%S)"
