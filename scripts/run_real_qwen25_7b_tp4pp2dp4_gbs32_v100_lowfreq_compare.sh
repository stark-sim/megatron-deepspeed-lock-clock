...
run_one() {
    ...
    if [[ "${mode}" == "baseline" ]]; then
        ...
        bash "${LAUNCH_SCRIPT}" 2>&1 | tee "/tmp/qwen7b_tp4pp2dp4_gbs32_lowfreq_baseline_$(date +%Y%m%d_%H%M%S).log"
        return
    fi
    ...
    TRAIN_STEPS="${TRAIN_STEPS}" \
    bash "${LAUNCH_SCRIPT}" 2>&1 | tee "/tmp/qwen7b_tp4pp2dp4_gbs32_lowfreq_static${clock}_$(date +%Y%m%d_%H%M%S).log"
}
