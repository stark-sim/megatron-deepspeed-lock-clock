...
LAUNCH_SCRIPT="${BASE_PATH}/scripts/run_eth_qwen7b_tp2pp2dp2.sh"
...
MASTER_PORT_BASE="${MASTER_PORT_BASE:-32251}"
...
run_one() {
    ...
    if [[ "${mode}" == "baseline" ]]; then
        experiment_name="eth_qwen7b_tp2pp2dp2_baseline_${RUN_TAG}"
        MASTER_PORT="${port}" \
    ...
    experiment_name="eth_qwen7b_tp2pp2dp2_static${clock}_${RUN_TAG}"
    MASTER_PORT="${port}" \
    EXPERIMENT_NAME="${experiment_name}" \
