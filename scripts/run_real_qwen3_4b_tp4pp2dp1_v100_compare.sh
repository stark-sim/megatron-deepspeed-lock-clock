...
STEPS="${STEPS:-20}"
CKPT_PATH="${CKPT_PATH:-${BASE_PATH}/checkpoints/qwen3_4b_hf2megads_tp4pp2_latest}"

# Verify checkpoint exists
if [[ ! -d "${CKPT_PATH}" ]]; then
    echo "[WARN] Checkpoint not found: ${CKPT_PATH}" >&2
    echo "[WARN] Please run convert_qwen3_4b_hf_to_megatron_v100.sh 4 2 first." >&2
fi

BASE_PORT=31631

echo "========================================"
echo "Qwen3-4B TP4PP2DP1 Baseline + Static Sweep"
...
EXPERIMENT_NAME="ib_real_qwen3_4b_tp4pp2dp1_baseline_formal${STEPS}_finetune_nosave" \
EXPERIMENT_MODE=baseline \
MASTER_PORT="${BASE_PORT}" \
TRAIN_STEPS="${STEPS}" \
LOAD_CHECKPOINT_PATH="${CKPT_PATH}" \
bash "${SCRIPT_DIR}/run_real_qwen3_4b_tp4pp2dp1_v100.sh"
...
for freq in ${FREQS}; do
    ...
    EXPERIMENT_NAME="ib_real_qwen3_4b_tp4pp2dp1_static${freq}_formal${STEPS}_finetune_nosave" \
