...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-ib_real_qwen25_7b_tp4pp2dp2_smoke5_finetune_nosave}"
...
export TP="${TP:-4}"
export PP="${PP:-2}"
# NOTE: TP4PP2DP2 requires 16 GPUs (TP*PP*DP=4*2*2=16)
export LOCAL_GPU_INDICES="${LOCAL_GPU_INDICES:-8,9,10,11,12,13,14,15}"
export DS_INCLUDE="${DS_INCLUDE:-v100x16-1:8,9,10,11,12,13,14,15@v100x16-2:8,9,10,11,12,13,14,15}"

export MASTER_ADDR="${MASTER_ADDR:-192.168.205.201}"
export MASTER_PORT="${MASTER_PORT:-31731}"
export HOSTFILE="${HOSTFILE:-/tmp/hostfile_real_qwen25_7b_tp4pp2dp2_v100}"
...
# NOTE: Must point to a TP4PP2 checkpoint (not TP2PP2).
export LOAD_CHECKPOINT_PATH="${LOAD_CHECKPOINT_PATH:-${BASE_PATH}/checkpoints/qwen25_7b_instruct_hf2megads_tp4pp2_20260506_214108}"
export FINETUNE="${FINETUNE:-1}"
export DISABLE_SAVE_CHECKPOINT="${DISABLE_SAVE_CHECKPOINT:-1}"
export NO_UNTIE_EMBEDDINGS="${NO_UNTIE_EMBEDDINGS:-1}"
