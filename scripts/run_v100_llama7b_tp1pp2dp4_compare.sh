...
echo "========================================"
echo "LLaMA7B Dual-Node TP1PP2DP4 Baseline + Static Sweep"
echo "Checkpoint: ${CKPT_PATH}"
echo "Freqs: ${FREQS} | Steps: ${STEPS}"
echo "========================================"

EXPERIMENT_NAME="ib_llama7b_tp1pp2dp4_baseline_formal${STEPS}_finetune_nosave" \
EXPERIMENT_MODE=baseline \
MASTER_PORT="${BASE_PORT}" \
TRAIN_STEPS="${STEPS}" \
LOAD_CHECKPOINT_PATH="${CKPT_PATH}" \
bash "${SCRIPT_DIR}/run_v100_llama7b_tp1pp2dp4.sh"

i=2
for freq in ${FREQS}; do
    EXPERIMENT_NAME="ib_llama7b_tp1pp2dp4_static${freq}_formal${STEPS}_finetune_nosave" \
