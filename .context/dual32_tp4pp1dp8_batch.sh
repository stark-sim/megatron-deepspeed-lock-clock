...
run_one() {
  local clock="$1"
  local port="$2"
  local run_id="dual32_tp4pp1dp8_static${clock}_$(date +%Y%m%d_%H%M%S)_DGX2-1"
  local run_dir="$BASE/experiments/$run_id"
  local log_dir="$run_dir/logs"
  local dscfg="$BASE/.context/dsconfig_dual32_tp4pp1dp8_static${clock}.json"
...
JSON
  ...
  echo "[dual32] freq=$clock port=$port run_id=$run_id"
  lock_local "$clock"
  lock_remote "$clock"
  (cd "$BASE" && deepspeed --hostfile "$HOSTFILE" --num_nodes 2 --num_gpus 16 --master_addr "$MASTER_ADDR" --master_port "$port" pretrain_gpt.py \
    ...
    --experiment-name "dual32_tp4pp1dp8_static${clock}" \
    --experiment-root-dir "$BASE/experiments" \
    --bf16 2>&1 | tee "$log_dir/$run_id.log")
  reset_local
  reset_remote
  cleanup_procs
  echo "[dual32] completed freq=$clock run_id=$run_id"
}
for idx in "${!FREQS[@]}"; do
