...
# Fix frequency_mhz from run_id using dataclass replace
source_samples = []
for s in source_samples_raw:
    freq = _extract_freq_from_run_id(s.run_id)
    if freq and s.observed.frequency_mhz is None:
        new_observed = replace(s.observed, frequency_mhz=freq)
        s = replace(s, observed=new_observed)
...
# Debug: print available keys in first prediction point
if transfer_prediction.get('supported_predictions'):
    first_pred = transfer_prediction['supported_predictions'][0]
    print(f"\nDebug: Available keys in prediction: {list(first_pred.keys())}")
...
accuracy_points = []
print(f"\n=== Accuracy Assessment ===")
print(f"{'Freq':>6} | {'ObsTime':>8} | {'PredTime':>8} | {'TimeAPE':>8} | {'ObsEnergy':>10} | {'PredEnergy':>10} | {'EnergyAPE':>8}")
print("-" * 90)
...
for sample in target_samples:
    ...
    energy_ape = _ape(observed_total_energy_j, pred["estimated_total_energy_j"])
    ...
    point = {
        ...
        "predicted_total_energy_j": pred["estimated_total_energy_j"],
        "total_time_ape": time_ape,
        "total_energy_ape": energy_ape,
    }
    accuracy_points.append(point)
    print(f"{freq:>6} | {observed_total_time_s:>8.1f} | {pred['estimated_total_time_s']:>8.1f} | "
          f"{time_ape:>7.2%} | {observed_total_energy_j:>10.1f} | {pred['estimated_total_energy_j']:>10.1f} | "
          f"{energy_ape:>7.2%}")
...
accuracy_summary = {
    ...
    "total_energy_mape": _mean([p["total_energy_ape"] for p in accuracy_points]),
    "points": accuracy_points,
}

print("-" * 90)
time_mape_str = f"{accuracy_summary['total_time_mape']:.2%}" if accuracy_summary['total_time_mape'] else "N/A"
energy_mape_str = f"{accuracy_summary['total_energy_mape']:.2%}" if accuracy_summary['total_energy_mape'] else "N/A"
print(f"{'MAPE':>6} | {'':>8} | {'':>8} | {time_mape_str:>8} | {'':>10} | {'':>10} | {energy_mape_str:>8}")
