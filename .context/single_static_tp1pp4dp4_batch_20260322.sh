...
HOSTFILE=/tmp/hostfile_tp1pp4dp4_single
...
trap 'reset_local; cleanup_procs' EXIT
run_one() {
  local clock="$1"
  local run_id="single_tp1pp4dp4_static${clock}_20260322_$(date +%Y%m%d_%H%M%S)_DGX2-1"
  local run_dir="$BASE/experiments/$run_id"
  local log_dir="$run_dir/logs"
  local dscfg="$BASE/.context/dsconfig_single_tp1pp4dp4_static${clock}_20260322.json"
...
JSON
  echo "[single-batch] freq=$clock run_id=$run_id"
  lock_local "$clock"
  (cd "$BASE" && deepspeed --hostfile "$HOSTFILE" --num_nodes 1 --num_gpus 16 pretrain_gpt.py \
    ...
    --experiment-name "single_tp1pp4dp4_static${clock}_20260322" \
    --experiment-root-dir "$BASE/experiments" \
    --bf16 | tee "$log_dir/$run_id.log")
  reset_local
  cleanup_procs
  echo "[single-batch] completed freq=$clock run_id=$run_id"
}
