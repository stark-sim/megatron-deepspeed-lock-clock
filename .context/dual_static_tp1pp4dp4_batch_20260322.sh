...
HOSTFILE=/tmp/hostfile_tp1pp4dp4_dual
FREQS=(1185 1192 1200)
...
lock_remote() {
  local clock="$1"
  ssh 192.168.205.202 "sudo -n python3 - $clock <<'PY2'
import sys
sys.path.insert(0, '/home/sd/.local/lib/python3.10/site-packages')
import pynvml
clock=int(sys.argv[1])
...
reset_remote() {
  ssh 192.168.205.202 "sudo -n python3 - <<'PY2'
import sys
sys.path.insert(0, '/home/sd/.local/lib/python3.10/site-packages')
import pynvml
pynvml.nvmlInit()
...
PY2
  ssh 192.168.205.202 "python3 - <<'PY2'
import os, signal, subprocess
for line in subprocess.run(['ps','-ef'],capture_output=True,text=True).stdout.splitlines():
...
run_one() {
  local clock="$1"
  local port="$2"
  local run_id="dual_tp1pp4dp4_static${clock}_20260322_$(date +%Y%m%d_%H%M%S)_DGX2-1"
  local run_dir="$BASE/experiments/$run_id"
  local log_dir="$run_dir/logs"
  local dscfg="$BASE/.context/dsconfig_dual_tp1pp4dp4_static${clock}_20260322.json"
...
JSON
  ...
  echo "[dual-batch] freq=$clock port=$port run_id=$run_id"
  lock_local "$clock"
  lock_remote "$clock"
  (cd "$BASE" && deepspeed --hostfile "$HOSTFILE" --num_nodes 2 --num_gpus 8 --master_addr "$MASTER_ADDR" --master_port "$port" pretrain_gpt.py \
    ...
    --experiment-name "dual_tp1pp4dp4_static${clock}_20260322" \
    --experiment-root-dir "$BASE/experiments" \
    --bf16 | tee "$log_dir/$run_id.log")
  reset_local
  reset_remote
  cleanup_procs
  echo "[dual-batch] completed freq=$clock run_id=$run_id"
}
for idx in "${!FREQS[@]}"; do
