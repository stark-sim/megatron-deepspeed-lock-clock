...
\usepackage{booktabs}
\usepackage{tikz}
% Charts drawn with pure TikZ (no pgfplots dependency)
...
\textbf{行业痛点:}
\begin{itemize}
    \item GPT-4级别模型训练耗电可达\textbf{1,200万度}
    \item 电费占总成本的\textbf{40-60\%}
    \item 碳排放成为AI发展的社会责任约束
\end{itemize}
\vspace{0.5cm}
\textbf{传统方案局限:}
\begin{itemize}
...
\begin{column}{0.5\textwidth}
\begin{block}{我们的洞察}
\textbf{固定频率策略}可以在不显著影响训练时间的前提下，实现显著的能耗节省。
\end{block}
\vspace{0.3cm}
\begin{tikzpicture}[scale=0.8]
    ...
    \node at (2.5,-0.5) {\small \textbf{节省25-35\%能耗}};
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{核心技术问题}
\begin{block}{核心挑战}
如何在\textbf{保证训练效率}的前提下，找到\textbf{能效最优}的GPU固定频率？
\end{block}
\vspace{0.3cm}
\begin{columns}
...
\begin{tikzpicture}[
    ...
    \node[right=0.3cm of mon,align=left,font=\small] {Zeus功率统计\\实时能耗追踪};
...
\begin{itemize}
    ...
    \item \textbf{监控层}: 毫秒级功率采集，精确到每个训练step
    \item \textbf{预测层}: 动态网络检测，自适应跨节点通信建模
\end{itemize}
\end{frame}

\begin{frame}{关键创新: 动态网络感知预测}
\begin{columns}
\begin{column}{0.5\textwidth}
\textbf{传统方案:}
\begin{itemize}
...
\begin{tabular}{ccccc}
\toprule
\textbf{拓扑} & \textbf{Baseline功率} & \textbf{优化频点} & \textbf{优化后功率} & \textbf{节省} \\
\midrule
...
\begin{itemize}
    ...
    \item 能效比(Tokens/J)提升\textbf{30-50\%}
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\begin{tikzpicture}[scale=0.6]
    ...
    \draw[->] (0,0) -- (0,4.5) node[above] {功耗 (W)};
...
\begin{frame}{预测准确性: 从98.5\%到11.48\%}
\begin{columns}
\begin{column}{0.5\textwidth}
\textbf{修复前(固定系数):}
\begin{itemize}
...
\vspace{0.5cm}
\begin{block}{技术价值}
预测准确性的提升使得\textbf{零样本优化}成为可能，减少实际验证所需的实验次数50\%+
\end{block}
\end{frame}
...
\begin{frame}{成本节省估算}
\begin{center}
\large \textbf{以100卡集群训练GPT-3级别模型为例}
\end{center}
\vspace{0.3cm}
\begin{columns}
\begin{column}{0.5\textwidth}
...
&= \textcolor{successgreen}{\textbf{6.48万元}}
\end{align*}
\end{column}
\begin{column}{0.5\textwidth}
\begin{tikzpicture}[scale=0.8]
    \node[draw,rectangle,rounded corners,fill=accentgold!20,minimum width=4cm,minimum height=1cm] (baseline) at (0,4) {Baseline: 30\%额外能耗};
    ...
    \node[draw,rectangle,rounded corners,fill=white,minimum width=4cm,minimum height=1cm] (savings) at (0,0) {单次训练节省: \textbf{6.5万元}};
...
\begin{block}{技术亮点}
\begin{itemize}
    \item \textbf{首创}动态网络感知的跨节点性能预测框架
    \item 实现从\textbf{98.5\%到11.48\%}的预测精度飞跃
    \item 支持\textbf{零样本}频率优化，减少实验成本50\%+
\end{itemize}
\end{block}
\end{frame}
