#!/usr/bin/env bash
# =============================================================================
# 等待系统负载下降后自动重采 1080/1155 MHz
# =============================================================================
set -euo pipefail

BASE="/home/sd/Megatron-DeepSpeed"
PEER="192.168.205.202"
SESSION="dual8_tp4pp1dp2_recollect_20260402"
...
echo "============================================" | tee -a "$LOG"
echo "[watch] $(date '+%Y-%m-%d %H:%M:%S') Starting watch loop" | tee -a "$LOG"
echo "============================================" | tee -a "$LOG"

# 轮询等待系统负载下降且 nsys stats 消失
for attempt in {1..180}; do
    NSYS_RUNNING=$(ps aux | grep 'nsys stats' | grep -v grep | wc -l || true)
    LOAD_AVG=$(cat /proc/loadavg | awk '{print $1}')
    LOAD_OK=$(echo "$LOAD_AVG < 6.0" | bc -l)
    
    if [[ "$NSYS_RUNNING" -eq 0 && "$LOAD_OK" -eq 1 ]]; then
        echo "[watch] System clean at $(date). loadavg=$LOAD_AVG, no nsys stats. Proceeding." | tee -a "$LOG"
        break
    fi
    
    echo "[watch] $(date) nsys_running=$NSYS_RUNNING loadavg=$LOAD_AVG ... waiting" | tee -a "$LOG"
    sleep 60
done

# 再次执行预检
echo "[watch] Running preflight before recollection..." | tee -a "$LOG"
...
echo "" | tee -a "$LOG"
echo "============================================" | tee -a "$LOG"
echo "[collect] All re-collection frequencies completed." | tee -a "$LOG"
echo "============================================" | tee -a "$LOG"
