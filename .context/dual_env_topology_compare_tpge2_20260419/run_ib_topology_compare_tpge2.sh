...
BATCH_ID="ib_topology_compare_tpge2_kv4_20260419"
...
export TORCH_EXTENSIONS_DIR="/dev/shm/megatron_tpge2_compare_20260419/torch_extensions_v100"
export TMPDIR="/dev/shm/megatron_tpge2_compare_20260419/tmp"
export PYTHONPYCACHEPREFIX="/dev/shm/megatron_tpge2_compare_20260419/pycache"
...
run_one() {
    ...
    run_id="${BATCH_ID}_${topo_name}_${run_ts}_v100x16-1"
    ...
    unset RUN_ID HOSTFILE VALIDATE_ONLY TP PP MASTER_PORT
}
...
for idx in "${!TOPO_NAMES[@]}"; do
    run_one "${TOPO_NAMES[$idx]}" "${TPS[$idx]}" "${PPS[$idx]}" "${PORTS[$idx]}"
    sleep 10
done
