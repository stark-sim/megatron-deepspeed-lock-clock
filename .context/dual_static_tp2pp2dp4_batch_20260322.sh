...
HOSTFILE=/tmp/hostfile_tp2pp2dp4_dual
FREQS=(1072 1080 1087)
...
run_one() {
  local clock="$1"
  local port="$2"
  local run_id="dual_tp2pp2dp4_static${clock}_20260322_$(date +%Y%m%d_%H%M%S)_DGX2-1"
  local run_dir="$BASE/experiments/$run_id"
  local log_dir="$run_dir/logs"
  local dscfg="$BASE/.context/dsconfig_dual_tp2pp2dp4_static${clock}_20260322.json"
...
JSON
  ...
  echo "[batch] freq=$clock port=$port run_id=$run_id"
  lock_local "$clock"
  lock_remote "$clock"
  (cd "$BASE" && deepspeed --hostfile "$HOSTFILE" --num_nodes 2 --num_gpus 8 --master_addr "$MASTER_ADDR" --master_port "$port" pretrain_gpt.py \
    ...
    --experiment-name "dual_tp2pp2dp4_static${clock}_20260322" \
    --experiment-root-dir "$BASE/experiments" \
    --bf16 | tee "$log_dir/$run_id.log")
  reset_local
  reset_remote
  cleanup_procs
  echo "[batch] completed freq=$clock run_id=$run_id"
}
for idx in "${!FREQS[@]}"; do
