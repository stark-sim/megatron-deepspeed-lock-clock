...
HOSTFILE=/tmp/hostfile_tp4pp1dp4_dual16
FREQS=(990 1080 1155)
...
run_one() {
  local clock="$1"
  local port="$2"
  local run_id="dual16_tp4pp1dp4_static${clock}_$(date +%Y%m%d_%H%M%S)_DGX2-1"
  local run_dir="$BASE/experiments/$run_id"
  local log_dir="$run_dir/logs"
  local dscfg="$BASE/.context/dsconfig_dual16_tp4pp1dp4_static${clock}_20260325.json"
...
JSON
  ...
  echo "[dual16] freq=$clock port=$port run_id=$run_id"
  lock_local "$clock"
  lock_remote "$clock"
  ...
  reset_remote
  cleanup_procs
  echo "[dual16] completed freq=$clock run_id=$run_id"
}
for idx in "${!FREQS[@]}"; do
