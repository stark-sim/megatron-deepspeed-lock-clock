#!/usr/bin/env bash
# =============================================================================
# 2x4 三频 source 数据采集脚本（修复版）
# - 强制 GPU 8-11（通过 DS_INCLUDE）
# - 共享 ds_config（解决多节点路径问题）
# - 集成预检脚本
# - 严格串行执行
# =============================================================================
set -euo pipefail
...
SESSION="dual8_tp4pp1dp2_collect_20260402"
...
# 确保 .deepspeed_env 包含 OOM 缓解配置
if [[ ! -f "$BASE/.deepspeed_env" ]] || ! grep -q "PYTORCH_CUDA_ALLOC_CONF" "$BASE/.deepspeed_env"; then
    echo "PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True" >> "$BASE/.deepspeed_env"
    echo "[setup] Appended PYTORCH_CUDA_ALLOC_CONF to $BASE/.deepspeed_env" | tee -a "$LOG"
fi
...
# ---------------------------------------------------------------------------
echo "============================================" | tee -a "$LOG"
echo "[preflight] $(date '+%Y-%m-%d %H:%M:%S')" | tee -a "$LOG"
echo "============================================" | tee -a "$LOG"

PREFLIGHT_OK=true
if [[ -x "$BASE/scripts/preflight_check.sh" ]]; then
    if ! "$BASE/scripts/preflight_check.sh" --node-list "$NODE_LIST" --gpu-indices "$TARGET_GPUS" --verbose >> "$LOG" 2>&1; then
        PREFLIGHT_OK=false
    fi
else
    echo "[warn] preflight_check.sh not found; skipping automated preflight" | tee -a "$LOG"
fi

if [[ "$PREFLIGHT_OK" == "false" ]]; then
    echo "[FATAL] Preflight checks failed. Aborting collection." | tee -a "$LOG"
    exit 1
fi

echo "[preflight] All checks passed. Starting sequential collection." | tee -a "$LOG"
...
for CLOCK in "${FREQUENCIES[@]}"; do
    RUN_ID="dual8_tp4pp1dp2_static${CLOCK}_$(date +%Y%m%d_%H%M%S)_DGX2-1"
...
JSON
    ...
    deepspeed \
        ...
        --experiment-name "dual8_tp4pp1dp2_static${CLOCK}" \
...
echo "" | tee -a "$LOG"
echo "============================================" | tee -a "$LOG"
echo "[collect] All frequencies completed." | tee -a "$LOG"
echo "============================================" | tee -a "$LOG"
